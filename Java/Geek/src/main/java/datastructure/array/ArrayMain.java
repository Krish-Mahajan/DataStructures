...
public class ArrayMain<T extends Comparable<T>> {
  ...
  /**
   * REMEMBER Function to search in sorted array using BinarySearch
   *
   * @param arr
   * @param key
   * @return
  ...
  private boolean binarySearch_util(T[] arr, T key, int startIndex, int endIndex) {
    ...
    if (arr[midIndex].compareTo(key) == 0) return true;
      ...
      return binarySearch_util(arr, key, midIndex + 1, endIndex);
    else return binarySearch_util(arr, key, startIndex, midIndex - 1);
  }

  /**
   * REMEMBEr https://www.geeksforgeeks.org/largest-sum-contiguous-subarray/
   *
   * @param a
   */
  public void maxSubArraySum(int a[]) {
    ...
    System.out.println("Maximum contiguous sum is " + max_sum_so_far);
  ...
  public void leftrotate(T[] arr ,int d,int n) {
	  for(int i = 0 ; i < d ; i++) leftrotatebyone(arr ,d ,n);
  }

  private void leftrotatebyone(T[] arr,int d,int n){
	  T temp = arr[0];
